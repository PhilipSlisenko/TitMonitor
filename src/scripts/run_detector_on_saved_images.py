...
def main(self, path: str):
    ...
    rows = db_writer.select_images_between_timestamps(datetime(2021, 10, 13, 12, 00), datetime(2021, 10, 13, 16, 00), False)

    for row in rows:
        img = cv2.imread(os.paht.join(config.root_dir_to_save_images, row['filename']))
